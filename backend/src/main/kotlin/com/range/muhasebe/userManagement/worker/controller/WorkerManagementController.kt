...
class WorkerManagementController(
    ...
    override fun getAllRoles(): EnumEntries<WorkerPermissions> {
        return WorkerPermissions.entries
    }

    override fun givePermission(
        userId: UUID,
        permissionName: List<WorkerPermissions>
    ) {
        return workerManagerService.givePermissionsToWorker(userId, permissionName)
    }
}